...
if ($conn->connect_errno) {
    ...
    $ret = $conn->query($qry);
    if ($ret->num_rows > 0) {
        while ($row = $ret->fetch_assoc()) { //While will set a full row at a time that returned by the query
            // var_dump($row);
            $cname = $row['c_name'];
            ...
            $insid = $row['user_id'];
            $qy = "select name from users where user_id='" . $insid . "'";
            $res = $conn->query($qy);
            if ($res->num_rows > 0) {
...
?>
                ...
                <tr>
                    <td style="background: linear-gradient(92.11deg, #487EB0 30.92%, rgba(<?php echo rand(10, 255); ?>, <?php echo rand(180, 255); ?>, 95, 0.536913) 80.37%, rgba(<?php echo rand(177, 255); ?>, 111, <?php echo rand(50, 255); ?>, 0) 124.93%);"><?php echo $cname; ?> <br> <span style="font-size: 14pt;font-weight: 400;"><?php echo 'Offered By: ' . $insname; ?></span>
                        ...
                        <button><a href="./student/enrolcourse.php?instructor_id=<?php echo $insid; ?>&course_id=<?php echo $cid; ?>&c_name=<?php echo $cname; ?>&duration=<?php echo $duraton; ?>&price=<?php echo $price; ?>">Enroll</a></button>
                    </td>
                </tr>
<?php
            } else {
